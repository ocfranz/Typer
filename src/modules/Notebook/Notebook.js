...
const Notebook = ({}) => {
  ...
  const handleEditableKeyDown = (event) => {
    switch (event.keyCode) {
      case 13:
        ...
        newLine.children[0].addEventListener("keydown", (event) =>
          handleEditableKeyDown(event)
        );
        newLine.children[0].addEventListener("input", (event) =>
          handleEditableOnInput(event)
        );

        if (target.parentNode.parentNode.addEventListener) {
          target.parentNode.parentNode.addEventListener(
            "DOMNodeInserted",
            requestFocus,
            false
          );
        ...
        target.parentNode.parentNode.removeEventListener(
          "DOMNodeInserted",
          (event) => {}
        );
        break;
      case 8:
        ...
        if (target.innerText == "") {
          if (
            document.querySelector("#notebook").firstChild.children[0] !=
            event.target
          ) {
      ...
      case 40:
        if (
          document.querySelector("#notebook").lastChild.children[0] !=
          event.target
        ) {
          event.preventDefault();
  ...
  const handleEditableOnInput = (event) => {
    ...
    if (target.textContent == ":h2") {
      ...
      let newLine = document.createElement("div");
      const stringComponent = renderToString(
        <EditableWrapper type="heading-two">
          <Editable
            onKeyDown={(event) => {
              handleEditableKeyDown(event);
            }}
    ...
    if (target.textContent == ":ul") {
      ...
      newLine.children[0].children[1].addEventListener("keydown", (event) =>
        handleEditableKeyDown(event)
      );
      newLine.children[0].children[1].addEventListener("input", (event) =>
        handleEditableOnInput(event)
      );
